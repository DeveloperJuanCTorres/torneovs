...
$(document).ready(function() {
          var showRegionList = function() {
              ubigeos.forEach(function(ubigeo) {
                  if (ubigeo.provincia == '00' && ubigeo.distrito === '00') {
                      $(".departamento").append('<option class="item-dep" data-number='+ ubigeo.departamento + ' value=' + ubigeo.nombre + '>' + ubigeo.nombre + '</option>');
                  }
              });
          }

          var showProvinceList = function(departamentoId) {
              // console.log(departamentoId, 'hahga')
              ubigeos.forEach(function(ubigeo) {
                  if (ubigeo.departamento == departamentoId && ubigeo.provincia != 0 && ubigeo.distrito === '00') {
                      $(".provincia").append('<option class="item-prov" data-number='+ ubigeo.provincia + ' value=' + ubigeo.nombre + '>' + ubigeo.nombre + '</option>');
                  }
              });
          ...
          var showDistrictList = function(departamento, provincia) {
              // console.log(departamento, provincia,'jsjgdjsb')
              ubigeos.forEach(function(ubigeo) {
                  if (ubigeo.departamento == departamento && ubigeo.provincia == provincia && ubigeo.distrito != 0) {
                      $(".distrito").append('<option data-number='+ ubigeo.distrito + ' value=' + ubigeo.nombre + '>' + ubigeo.nombre + '</option>');
                  }
              });
          }
          ...
          $(".departamento").change(function() {
              $(".provincia").text('');
              $(".distrito").text('');
              var numeroId = $('.item-dep:selected').data('number');        
              showProvinceList(numeroId);
              var provincia = $(".provincia").val();
              console.log(numeroId,'provincia');
          })
  
          $(".provincia").change(function() {
              $(".distrito").text('');
              var departamento = $('.item-dep:selected').data('number');  
              var numeroId = $('.item-prov:selected').data("number");
              
              showDistrictList(departamento, numeroId);
              console.log(numeroId,'distrito');
          })
